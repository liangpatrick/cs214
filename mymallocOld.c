...
void *mymalloc(size_t size)
{
    ...
    case 2:;
        ...
        // while (ptr != NULL &&                 //casts pointer from size_t to size_t*
        //        ((*ptr & -2) - BOUNDS) <= size && //checks if payload fits between header and footer (& -2 is for redundancy free blocks should not have 1 low bit)
        //        (*ptr & -2) - ((payloadBlocks) + BOUNDS) <= MIN_REGION_SIZE)
        // {                                 //checks if resulting free region if split occurs is big enough to store pointers
        //     ptr = (size_t *)(*(ptr + 2)); //sets ptr to next ptr (stored in offset of 16) (pointer must be case from size_t to size_t*)
        ...
        while (ptr != NULL)
        {
            ...
            // if ((int)(*ptr & -2) + ((int)BOUNDS * 8) >= ((int)payloadBlocks * 8))
            // {
                // checks if free region is same size as payLoadBlocks
                // if it is the same, break out of loop so everything can be updated
                if (((int)(*ptr & -2) + ((int)BOUNDS * 8)) == ((int)payloadBlocks * 8))
                {
                    // printf("fuck me \n\n\n\n\n");
                    bestFit = ptr;
                ...
                // if not, then checks to see if its smallest so far, if so store.
                else if (((int)(*ptr & -2) < (int)(*bestFit & -2)) && !((int)(*ptr & -2) - (((int)payloadBlocks * 8) + ((int)BOUNDS * 8)) <= (int)MIN_REGION_SIZE))
...
void *moddedmalloc(size_t size, size_t *pointer, size_t *prevOfNextRegion, size_t *nextOfNextRegion)
{
    if (size == 0)
        return NULL;
...
void *myrealloc(void *ptr, size_t size)
{
    ...
    size_t *curRegionHead = (size_t *)ptr - 1;
    ...
    if ((nextRegionSize != 0 && (((*nextRegionHead) & 1) != 1)))
    ...
    size_t *newRegion = moddedmalloc(size, (size_t *)ptr - 1, prevOfNextRegion, nextOfNextRegion); //attempts to keep in place
    if (newRegion == NULL)
    {
